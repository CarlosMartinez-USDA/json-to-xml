...
-->
    ...
    <xsl:template name="name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
     ...
     <!--author given and family names-->
        <xsl:for-each select="map/array[@key='pub_authors']/map">
    ...
    <!--    <xsl:for-each select="map[position()]">
            <xsl:if test="string[@key = 'name'][position()]">
             <namePart type="given">
    ...
    <xsl:template name="relatedItem" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:variable name="pub_host" select="$nodes/usfs:research/usfs:seriesPublications/usfs:seriesPub[@type='host']/(usfs:abbrv|usfs:treePub)"/>
        <xsl:variable name="pub_series" select="$nodes/usfs:research/usfs:seriesPublications/usfs:seriesPub[@type='series']/(usfs:abbrv|usfs:treePub)"/>
        <xsl:choose>
    ...
    <xsl:apply-templates select="map/string[@key = 'pub_type_desc']" mode="titleInfo"/>
    </xsl:template>
    ...
    <xsl:template match="map/string[@key = 'pub_type_desc']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions" mode="titleInfo">
        <xsl:variable name="nodes">
            <xsl:copy-of select="document('./USFS_Research.xml')"/>
        </xsl:variable>
        <xsl:variable name="pub_host" select="$nodes/usfs:research/usfs:seriesPublications/usfs:seriesPub[@type='host']/(usfs:abbrv|usfs:treePub)"/>
        ...
        <xsl:choose>
            <xsl:when test=".=$pub_host">
              <titleInfo>
                ...
                <title>
                    <xsl:sequence select="f:seriesToAbbrv(.)"/>
                </title>
            </titleInfo>
            </xsl:when>
            <xsl:when test=".=$pub_series">
                <xsl:variable name="publication" select="map/string[@key='pub_publication']"/>
             <titleInfo>
    ...
    <xsl:template name="usfs_identifiers" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <identifier>
                <xsl:if test="map/string[@key = 'url_landing_page']">
                ...
                <xsl:if test="map/string[@key = 'url_binary_file']">
                    ...
                    <location>
                        <url access="raw object">
                            <xsl:attribute name="note" select="translate(@key, '_', ' ')"/>
                            <xsl:value-of select="normalize-space(string[@key = ('url_binary_file')])"/>
                        </url>
                    </location>
                </xsl:if>
        </identifier>
