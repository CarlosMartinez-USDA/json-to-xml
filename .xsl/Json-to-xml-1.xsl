...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="data">
        <xsl:result-document omit-xml-declaration="yes" indent="yes" encoding="UTF-8"
            href="file:///{$workingDir}{replace($originalFilename, '(.*/)(.*)(\.xml)', '$2')}_{position()}.json" format="archive">
            <xsl:copy-of select="."/>
        </xsl:result-document>
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            href="file:{$workingDir}N-{replace($originalFilename, '(.*/)(.*)(\.xml)', '$2')}_{position()}.xml" format="original">
            <mods version="3.7">
    ...
    <xsl:template match="data">
        <xsl:result-document omit-xml-declaration="yes" indent="yes" encoding="UTF-8"
            href="{$workingDirectory}{$archiveFile}_{position()}.xml" format="archive">
            <xsl:copy-of select="."/>
        </xsl:result-document>
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            href="{$workingDirectory}N-{$archiveFile}_{position()}.xml" format="original">
            <mods version="3.7">
                <xsl:apply-templates select="json-to-xml(.)"/>
            </mods>
        </xsl:result-document>
    ...
    <xsl:template match="map" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:apply-templates select="./string[@key='primary_station'] | ./array[@key = 'pub_authors']"/>
        ...
        <!--(1)abstract and (2)subject/topic-->
    ...
    <xd:doc>
        <!--author templates-->
            <xd:doc scope="component" id="contrib">
                <xd:desc>If the contributor is a collaborator rather than an individual, format output accordingly. If processing the first author in the group, assign an attribute of
                    <xd:b>usage:</xd:b> with a value of "primary."</xd:desc>
            </xd:doc>
            <xsl:template match="map/string[@key='primary_station'] | map/array[@key = 'pub_authors']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
                <xsl:choose>
        ...
        <xsl:for-each select="map[position()]">
            ...
            <xsl:choose>
                <xsl:when test="(./string[@key = 'station_id']!= ' ') and (./string[@key = 'unit_id']!= ' ')">
                    <affiliation>
                        <xsl:value-of select="$addUnitName"/>                        
                    </affiliation>
                </xsl:when>
...
<!--add host and conditonal here-->
   ...
   <xsl:param name="publication" as="xs:string" select="."/>
        <xsl:analyze-string select="map/string[@key='pub_publication']" regex="('Forest Insect &amp; Disease Leaflet')|('General Technical Report')|('General Technical Report - Proceedings')|('Information Forestry')|('Proceeding \(Rocky Mountain Research Station Publications\)')|('Resource Bulletin')|('Research Map')|('Research Note')|('Research Paper') | ('Miscellaneous Publication')">
   ...
   <!-- <xsl:template match="map/string[@key = 'pub_publication']">
    ...
    <xsl:template name="usfs_identifiers" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
    <identifier>
        <xsl:choose>
            <xsl:when test="map/string[@key = 'url_binary_file']">
                ...
                <location>
                    <url>
                        ...
                        <xsl:value-of select="normalize-space(string[@key=('url_binary-file')])"/>
                    </url>
                </location>
            </xsl:when>
    ...
    <xd:doc>
        <xd:desc> &lt;xsl:otherwise&gt;
            &lt;!-\- &lt;xsl:when test="local-name() = 'ELocationID'"&gt;-\-&gt;
            &lt;xsl:attribute name="type"&gt;
