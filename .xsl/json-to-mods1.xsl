...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="data">
        <xsl:result-document omit-xml-declaration="yes" indent="yes" encoding="UTF-8"
            href="file:///{$workingDir}A-{replace($originalFilename,'(.*/)(.*)(\.xml)', '$2')}_{position()}.json" format="archive">
           <xsl:copy-of select="."/>           
        </xsl:result-document>
    ...
    <xd:doc scope="component" id="main_title">
        <xd:desc>
            ...
            <xd:p><xd:b>output:</xd:b>outputs to a string value containing the main title of an article</xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xsl:template match="map/array[@key = 'pub_authors']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:choose>
            ...
            <xsl:otherwise>
                    <xsl:call-template name="author-name-info"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="author-name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:param name="unitName" as="xs:string*">
        ...
        <name type="personal">
            ...
            <!-\-affiliaton-\->
            <xsl:choose>
                <xsl:when test="usfs:unitNumToName != ' ') and (string[@key = 'unit_id'] != ' ')">
                <affiliation>
            ...
            <xsl:choose>
                <xsl:when test="(./string[@key = 'station_id'] != '') and (./string[@key='unit_id'] !='')">
                    <affiliation>
                        ...
                        <xsl:value-of select="usfs:acronymToName(fn:string[@key = 'station_id'])"/> 
                        <xsl:text>,</xsl:text>
                        <xsl:value-of select="usfs:unitNumToName(fn:string[@key = 'unit_id'])"/>
                        <xsl:text>,</xsl:text>
                        <xsl:value-of select="usfs:acronymToAddress(fn:string[@key = 'station_id'])"/>
                    </affiliation>
                </xsl:when>
                <xsl:when test="(./string[@key = 'station_id'] != '') and (./string[@key='unit_id'] ='')">
                    <affiliation>
                        ...
                        <xsl:text>,</xsl:text>
                        <xsl:value-of select="usfs:acronymToAddress(string[@key = 'station_id'])"/>
                    </affiliation>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:if test="(./string[@key='station_id'] ='') and (./string[@key = 'unit_id'] = '')"/>  
    ...
    <xsl:template match="map/string[@key = 'primary_station']"
        ...
        <xsl:if test=". and count(map/array[@key = 'pub_authors'] = 0)">
         <name type="corporate">
            <namePart>United States Department Agriculture</namePart>
    ...
    <xd:doc>
        <xd:desc><xd:ul><xd:p>Transforms, in order of preference, the publication-related date metadata. </xd:p>
            <xd:p>listed below:</xd:p>
            <xd:li>
                <xd:p>modified_on</xd:p>
            </xd:li>
            <xd:li>
                <xd:p>created_on</xd:p>
            </xd:li>
            <xd:li>
                <xd:p>prodcut_year</xd:p>
            </xd:li>
        </xd:ul>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc scope="component">
        <xd:desc>
            <xd:p><xd:b>vendorName</xd:b>Metadata supplier name (e.g., Brill, Springer,
                Elsevier)</xd:p>
            <xd:p><xd:b>archiveFile</xd:b>Filename from source metadata (eg. filename.xml,
