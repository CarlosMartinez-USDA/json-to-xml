...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:isodates="http://iso" exclude-result-prefixes="xs xd f isodates" version="2.0">
    ...
    <xsl:function name="f:distinct-nodes" as="node()*">
        ...
        <xsl:sequence select="
                ...
                return
                    $nodes[$seq][not(f:is-node-in-sequence(
                    ., $nodes[position() &lt; $seq]))]
                "/>
    ...
    <xsl:function name="f:getMonthNumFromName" as="xs:string">
        <xsl:param name="month-name" as="xs:string"/>
        <xsl:variable name="months"
            select="'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'November', 'December'"
            as="xs:string+"/>
        <xsl:variable name="seq" as="xs:string+">
            <xsl:sequence select="f:distinct-nodes($months)"/>
        </xsl:variable>

        <xsl:analyze-string select="normalize-space($month-name)"
            regex="([0-9]{{1,2}})\s([A-Z][a-z]+)\s([0-9]{{4}})">
            <xsl:matching-substring>
                <xsl:number value="regex-group(3)" format="01"/>
                <xsl:text>-</xsl:text>
                <xsl:number
                    value="format-number(index-of($months, lower-case(substring($month-name, 1, 3))), '00')"
                />"/> <xsl:text>-</xsl:text>
    ...
    <xsl:function name="f:checkMonthType">
        ...
        <xsl:choose>
            ...
            <xsl:when test="(string-length($testValue) > 2 and (not(contains($testValue, '–'))))">
                <xsl:value-of select="f:getMonthNumFromName($testValue)"/>
            </xsl:when>
        </xsl:choose>
    </xsl:function>
