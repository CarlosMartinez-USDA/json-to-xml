...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="seriesTitle" select="('Forest Insect &amp; Disease Leaflet', 'General Technical Report', 'General Technical Report - Proceedings', 'Information Forestry',
        'Proceeding (Rocky Mountain Station Publications)','Resource Bulletin (RB)', 'Research Map (RMAP)', 'Research Note (RN)', 'Research Paper (RP)', 'Resource Update (RU)')"/>       
 ...
 <!--United States Forest Service Research Naming Functions (UFSF RNF) -->
    ...
    <xsl:function name="f:seriesToAbbrv" as="xs:string?" xmlns:f="http://functions">    
        ...
        <xsl:value-of select="if ($nodes/usfs:research/../usfs:seriesPub[usfs:treePub = $typeOfTitle]/usfs:abbrv)  
            then ($nodes/usfs:research/../usfs:seriesPub[usfs:treePub = $typeOfTitle]/usfs:treePub)          
            else ($typeOfTitle)"/>
    </xsl:function>
    ...
    <xd:doc scope="component">
        <xd:desc>
            <xd:p><xd:b>Function: </xd:b>f:seriesToAbbrv</xd:p>
            <xd:p><xd:b>Usage: </xd:b>f:seriesToAbbrv(f:unitNumToName(string[@key = 'unit_id'])</xd:p>
            <xd:p><xd:b>Purpose: </xd:b>Convert ISO 639-2b three-letter codes into ISO 639-1 two-letter codes.</xd:p>            
        </xd:desc>
