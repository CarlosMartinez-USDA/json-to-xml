...
-->
    ...
    <xsl:template match="map" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <!--<!-\-<relatedItem>-\->
            <xsl:apply-templates select="./string[@key = 'pub_type_desc']" mode="related_info"/>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>author's given and family name is parsed from "name" string key value</xd:p>
            <xd:p>displayName matches on the 'name' string key value.</xd:p>
            <xd:p>affiliation uses two external stylesheets to match abbreviated station and unit numbers and names with their respective whole name and address</xd:p>
        ...
        <xsl:for-each select="map[position()]">
            ...
            <xsl:if test="./string[@key = 'station_id'] !=''">
                <affiliation>
                    ...
                    <xsl:when test="string(./string[@key='unit_id']) and ($unitAcronym != '')">
...
<xd:doc>
    ...
    <xsl:template match="map/string[@key='status_name']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">    
    ...
    <xsl:template name="pages" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
            ...
            <xsl:param name="pages" as="xs:string" select="tokenize($pub_publication,'[\d{{2}.]')[last()]"/>
                    ...
                    <xsl:when test="$pages">
                        ...
                        <xsl:analyze-string select="$pages" regex="(\d{{1,4}})(\s\w{{1,4}})|(\S{{1,4}})\-\S{{1,4}}">
                            ...
                            <xsl:non-matching-substring>
                                ...
                                <start>
                                    <xsl:value-of select="substring-after(substring-before($pub_publication,'.'[last()]),'\d{{2}}')"/>
                                </start>
                                <end>
