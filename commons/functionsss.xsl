...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!--usfs global variables-->
    <xsl:variable name="tree_nodes">
        <xsl:copy-of select="document('./usfs_treesearch.xml')"/>
    </xsl:variable>
    
    <xsl:variable name="seriesTitles" select="text()=('Forest Insect &amp; Disease Leaflet', 'General Technical Report (GTR)', 'General Technical Report - Proceedings', 'Information Forestry', 'Prsoceeding (Rocky Mountain Research Station Publications)',
 ...
 <!--United States Forest Service Research Naming Functions (UFSF RNF) -->
    ...
    <xsl:function name="f:acronymToName">
        ...
        <xsl:value-of select="$tree_nodes/usfs:research/usfs:stations/usfs:station[usfs:acronym = $acronym]/usfs:name"/>
    </xsl:function>
    ...
    <xsl:function name="f:acronymToAddress" as="xs:string" xmlns:f="http://functions">
        <xsl:param name="acronym"/>
        <xsl:if test="$acronym != ''"/>
        <xsl:value-of select="$tree_nodes/usfs:research/usfs:stations/usfs:station[usfs:acronym = $acronym]/usfs:address"/>
    </xsl:function>
    ...
    <xsl:function name="f:unitNumToName" as="xs:string" xmlns:f="http://functions">
        ...
        <xsl:if test="$unitNum != ''"/>
        <xsl:value-of select="$tree_nodes/usfs:research/usfs:stations/usfs:station/usfs:researchUnits/usfs:researchUnit[usfs:unitNumber=$unitNum]/usfs:unitName"/>
        </xsl:function>
    ...
    <xsl:function name="f:unitAcronymToName" as="xs:string" xmlns:f="http://functions">
        ...
        <xsl:value-of select="$tree_nodes/usfs:research/usfs:stations/usfs:station/usfs:researchUnits/usfs:researchUnit[usfs:unitAcronym= $unitAcronym]/usfs:unitName"/>
    </xsl:function>
    ...
    <xsl:function name="f:seriesToAbbrv" as="xs:string" xmlns:f="http://functions">
       ...
       <xsl:if test="$seriesTitle != ''"/>
        ...
        <xsl:value-of select="if ($nodes/usfs:treesearch/usfs:treesearchPublications/usfs:treePub[usfs:pubTitle = $typeOfTitle])  
       ...
       -->
        ...
        <xsl:function name="f:seriesToAbbrv" as="xs:string" xmlns:f="http://functions">
            <xsl:param name="typeOfTitle"/>
            <xsl:if test="$typeOfTitle != ''"/>
            <xsl:value-of select="if ($tree_nodes/usfs:treesearch/usfs:treesearchPublications/usfs:treePub[usfs:pubTitle = $typeOfTitle])  
    ...
    <xsl:function name="f:pub_type_desc" as="xs:string" xmlns:f="http://functions">
        ...
        <xsl:choose>
            <xsl:when test="matches($pub_title,$nodes/usfs:research/usfs:treeSeries/usfs:seriesPub[@type='series'])">
                <xsl:sequence select="$tree_nodes/usfs:research/usfs:treeSeries/usfs:seriesPub[usfs:treePub = $pub_title]/usfs:abbrv"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:sequence select="$tree_nodes/usfs:research/usfs:treeSeries/usfs:seriesPub[usfs:treePub= $pub_title]"/> 
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
</xsl:stylesheet>