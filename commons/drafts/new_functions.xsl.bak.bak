...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="f:getMonthNumFromName" as="xs:string">
        <xsl:param name="month-name" as="xs:string"/>
        <xsl:variable name="months" select="'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'November', 'December'" as="xs:string+"/>
        <xsl:variable name="seq" as="xs:string+">
        <xsl:sequence select="f:distinct-nodes($months)"/>
        </xsl:variable>
        
    <xsl:analyze-string select="normalize-space($month-name)" regex="([0-9]{{1,2}})\s([A-Z][a-z]+)\s([0-9]{{4}})">
        <xsl:matching-substring>
            <xsl:number value="regex-group(3)" format="01"/>          
            <xsl:text>-</xsl:text>
            <xsl:number value="format-number(index-of($months, lower-case(substring($month-name, 1, 3))), '00')"/>"/>
            <xsl:text>-</xsl:text>
