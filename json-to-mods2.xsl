...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:isodates="http://iso" xmlns:usfs="http://usfsresearch"
    exclude-result-prefixes="xs xd f isodates" version="2.0">
    ...
    <xd:doc scope="component">
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>Pull the two-digit equivalent of a month name for a given
                date string. e.g. "June 28, 2017" will return "06." The name of the month
    ...
    <xd:doc scope="component">
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>If no day provided, return nothing. If day provided, pad
                with zeroes to 2 digits.</xd:p>
        </xd:desc>
    ...
    <xd:doc scope="component">
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>Convert ISO 639-2b three-letter codes into ISO 639-1
                two-letter codes.</xd:p>
        </xd:desc>
    ...
    <xsl:function name="f:isoTwo2One" as="xs:string">
        ...
        <xsl:sequence
            select="$nodes/isodates:isodates/isodates:value[isodates:two = $isoTwo]/isodates:one"/>
    </xsl:function>
    ...
    <xsl:function name="f:acronymToName">
        ...
        <xsl:sequence select="$nodes/usfs:research/usfs:stationCodes/usfs:stationCode[usfs:acronym = $acronym]/usfs:name" />
    </xsl:function>
  

    <xd:doc scope="component" id="f:acronymToAddress" >
        <xd:desc>
            <xd:p><xd:b>Function: </xd:b>f:acronymToName</xd:p>
            <xd:p><xd:b>Usage: </xd:b>f:acronymToName(usfs station code)</xd:p>
        </xd:desc>
        <xd:param name="acronym"/>
    </xd:doc>
    <xsl:function name="f:acronymToAddress" as="xs:string*" xmlns:f="http://functions">
        <xsl:param name="acronym" as="xs:string?"/>
        <xsl:variable name="nodes">
            <xsl:copy-of select="document('./usfsResearchStations.xml')"/>
        </xsl:variable>       
            <xsl:value-of select="$nodes/usfs:research/usfs:stationCodes/usfs:stationCode[usfs:acronym = $acronym]/usfs:agency"/>
            <xsl:value-of select="$nodes/usfs:research/usfs:stationCodes/usfs:stationCode[usfs:acronym = $acronym]/usfs:name"/>
            <xsl:value-of select="$nodes/usfs:research/usfs:stations/usfs:station/usfs:researchUnits/usfs:researchUnit[usfs:unitNumber = $acronym]/usfs:unitName"/>
            <xsl:value-of select="$nodes/usfs:research/usfs:stationCodes/usfs:stationCode[usfs:acronym = $acronym]/usfs:address"/>
    </xsl:function>
...
<!-\-       <xsl:sequence select="$nodes/usfs:research/usfs:stations/usfs:station/usfs:researchUnits/usfs:researchUnit[usfs:acronym = ($unitAcronym, $unitNum)]/usfs:unitName"/>-\->
        <xsl:sequence select="$nodes/usfs:research/usfs:stations/usfs:station/usfs:researchUnits/usfs:researchUnit[usfs:unitNumber = $unitNum]/usfs:unitName"/>   
    </xsl:function>-->
    ...
    <xd:doc>
        <xd:desc/>
        <xd:param name="originalString"/>
        <xd:param name="stringToInsert"/>
        <xd:param name="pos"/>
    </xd:doc>
    <xsl:function name="f:insert-string" as="xs:string">
