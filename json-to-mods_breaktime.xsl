...
<!--            <xsl:when test="string[@key = 'pub_page'] except *[($start_page) or ($end_page)]">
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                        ...
                        <xsl:matching-substring>
                            <xsl:choose>
                                <xsl:when test="regex-group(1) and not(regex-group(2))">
                                    <xsl:comment>subtest 2a</xsl:comment>
                                    <total>
                                        <xsl:value-of select="regex-group(1)"/>
                                    </total>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:comment>subtest 2b</xsl:comment>
                                    <start>
                                    <xsl:value-of select="regex-group(1)"/>
                                    </start>
                                    <end>
                                    ...
                                    <total>
                                        <xsl:value-of select="f:calculateTotalPgs(regex-group(1),substring-before(regex-group(2),'-'))"/>
                                    </total>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:matching-substring>
                    ...
                    <!--  <xsl:analyze-string select="$citation" regex="[^\d](\d+)(-\d+)?(\sp|\sp$)|(pages\s|Pages\s|:\s|p\.\s)(\d+-\d+)">
                        ...
                        <xsl:fallback>
                            <xsl:apply-templates select="/fn:map/fn:string[@key = 'citation']" mode="start_end"/>
                        </xsl:fallback>
                    </xsl:analyze-string>
                   <xsl:when test="regex-group(1) and regex-group(2)">
                                    <xsl:comment>subtest 3b</xsl:comment>
                                    <start>
                                        <xsl:value-of select="substring-before(regex-group(2), '-')"
                                        />
                                    </start>
                                    ...
                                    <total>
                                        <xsl:variable name="first" select="number(substring-before(regex-group(2), '-'))"/>
                                        <xsl:variable name="last" select="number(substring-after(regex-group(2), '-'))"/>
...
<!-\-                        <xsl:comment>test 4</xsl:comment>-\->
                <extent unit="pages">
                    <xsl:analyze-string select="/map/string[@key = 'pub_page']"
                        ...
                        <xsl:non-matching-substring>
                            <xsl:apply-templates select="$citation" mode="page_numbers_from_citation"/>
                        </xsl:non-matching-substring>
                    </xsl:analyze-string>
                </extent>
            </xsl:when>
