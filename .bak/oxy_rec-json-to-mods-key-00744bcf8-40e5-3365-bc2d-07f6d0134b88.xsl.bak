...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:key name="series_publications" match="map/string[@key = 'pub_type_desc']" use="document('../commons/usfs_treesearch.xml')/usfs:research/../usfs:treePub[@type='series']"/>
    <xd:doc>
        <xd:desc/>
    </xd:doc>
...
<!--Result document template-->
    ...
    <xsl:template match="data">
        <xsl:result-document omit-xml-declaration="yes" indent="no" method="json"
            href="{$workingDire}{$archiveFile}_{position()}.json"
            format="archive">
            <xsl:copy-of select="."/>
        </xsl:result-document>
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            href="{$workingDire}N-{$archiveFile}.xml"
            format="original">
            <mods>
    ...
    <!--Template uses XML transformed from JSON (i.e. json-to-xml(.))-->
    <xd:doc>
        <xd:desc>
            <xd:p/>
        </xd:desc>
...
<!--Article Title-->
    ...
    <xsl:template name="name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:variable name="acronym" select="map[position()]/array[@key='pub_authors']/map/string[@key='station_id']"/>
        <xsl:variable name="unitNum" select="map[position()]/array[@key='pub_authors']/map/string[@key='unit_id']"/>" 
        <xsl:variable name="unitAcronym" select="map[position()]/array[@key='pub_authors']/map/string[@key='unit_id']"/>"
        ...
        <xsl:if test="string[@key = 'name']">
            ...
            <xsl:if test="contains($acronym, text()) and .!=''">
                <affiliation>
                    <xsl:text>U.S. Department of Agriculture, Forest Service,</xsl:text>
                    <xsl:value-of select="local:acronymToName($acronym)"/>
                    <xsl:text>, </xsl:text>
                    <xsl:choose>
                        <xsl:when test="number($unitNum) > 0">                        
                             <xsl:value-of select="local:unitNumToName($unitNum)"/>
                             <xsl:text>, </xsl:text>
                        </xsl:when>
                        <xsl:when test="contains($unitAcronym, text()) and .!=''">
   ...
   <!-- <xsl:template name="name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:for-each select="map[position()]/string">
            <xsl:if test="@key = 'name'">
                ...
                <xsl:if test="@key = 'station_id' and .!=''">
                    <affiliation>
                        <xsl:text>United States Department of Agriculture, Forest Service, </xsl:text>
                        <xsl:value-of select="local:acronymToName(@key = 'station_id')"/>
                        <xsl:text>, </xsl:text>
                        <xsl:choose>
                            <xsl:when test="@key = 'unit_id' and .!= ''">
                                <xsl:text>, </xsl:text>
                                <xsl:value-of select="local:unitNumToName(@key = 'unit_id')"/>
                            </xsl:when>
                            <xsl:otherwise>   
                            <xsl:text>, </xsl:text>
                                <xsl:value-of select="local:unitAcronymToName(@key = 'unit_id')"/>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:text>, </xsl:text>
    ...
    <xd:doc>
        <xd:desc>to match and creaete apply-templates for corporate body</xd:desc>
    </xd:doc>
    ...
    <xsl:template match="map/string[@key = ('pub_type_desc', 'pub_publication','citation')]"
        ...
        <xsl:variable name="pub_type_desc" select="string[@key = 'pub_type_desc']"/>
        <xsl:variable name="pub_publication" select="string[@key = 'pub_publication']"/>        
        <xsl:variable name="pub_citation" select="string[@key = 'citation']"/>
        <xsl:for-each select="*">
            <xsl:choose>
                <xsl:when test="contains($p_series, string[@key = 'pub_type_desc'])">
                    <relatedItem type="series">
                        <titleInfo type="abbreviated">
                            <title>
                                <xsl:value-of select="key('series_publications', $pub_type_desc)"/>
<!--                                <xsl:apply-templates select="$pub_type_desc" mode="relatedItem_title"/>-->
                            </title>
                        </titleInfo>
                    </relatedItem>
                </xsl:when>
    ...
    <xsl:template
        ...
        <xsl:if test="./string[@key = 'url_landing_page']">
            ...
            <xsl:if test="./string[@key = 'url_binary_file']">
                <identifier>
                    <xsl:attribute name="type">
                        <xsl:value-of select="translate(@key, '_', ' ')"/>
                    </xsl:attribute>
                    <xsl:value-of
                        select="tokenize(./string[@key = 'url_binary_file'], '/')[last()]"/>
                </identifier>            
            <location>
                <url access="raw object">
                    <xsl:value-of select="./string[@key = 'url_binary_file']"/>
                </url>
            </location>
        </xsl:if>
    </xsl:template>
