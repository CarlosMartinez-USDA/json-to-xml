...
-->
    ...
    <xsl:template match="map/string[@key = 'abstract']"  xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <abstract>
    ...
    <xsl:template match="map/array[@key = 'national_research_taxonomy_elements']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:for-each select="map[position()]">
    ...
    <xsl:template match="map/string[@key = 'pub_type_desc']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:param name="relatedItem" select="."/>
        <xsl:variable name="publications">
            <xsl:sequence select="string(concat('Forest Insect &amp; Disease Leaflet', 'General Technical Report', 'General Technical Report - Proceedings', 'Information Forestry', 'Proceeding Rocky Mountain Research Station Publications', 'Resource Bulletin', 'Research Map', 'Research Note', 'Research Paper'))"/>
        </xsl:variable>
        <xsl:variable name="publication" select="tokenize($publications,',')"/> 
        <xsl:variable name="nodes">
            <xsl:copy-of select="document('./USFS_Research.xml')"/>
        </xsl:variable>
        <xsl:variable name="pub_host" select="$nodes/usfs:research/usfs:seriesPublications/usfs:seriesPub[@type='host']/usfs:treePub"/>
        <xsl:variable name="pub_series" select="$nodes/usfs:research/usfs:seriesPublications/usfs:seriesPub[@type='series']/usfs:treePub"/>
        <relatedItem>
            <xsl:choose>
            <xsl:when test="$pub_host">
                <xsl:attribute name="type">host</xsl:attribute>
            </xsl:when>
            <xsl:when test="contains($relatedItem, $pub_series)">
            ...
            <xsl:for-each select="$relatedItem">
                <xsl:choose>
                <xsl:when test="contains(.,$pub_host)">
                    <xsl:variable name="pub_publication" select="map/string[@key='pub_publications']"/>
                <titleInfo type="alternative">
                  ...
                  <xsl:when test="contains(.,$pub_series)">
    ...
    <xsl:template match="map/string[@key = 'pub_type_desc']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:if test="contains(.,$pub_series)">
            <titleInfo>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                    ...
                    <titleInfo>
                        <xsl:attribute name="type">alternative</xsl:attribute>
                        <title><xsl:copy-of select="(substring-before(tokenize($publication), '[.,:]\s+'),following-sibling::node())"/></title>
                    </titleInfo>
                ...
                <xsl:when test="map/string[@key = 'url_binary_file']">
                    ...
                    <location>
                        <url access="raw object">
                            ...
                            <xsl:value-of
                                select="normalize-space(string[@key = 'url_landin_page'])"/>
                        </url>
                    </location>
                </xsl:when>
                    <xsl:when test="map/string[@key = 'url_binary_file']">
