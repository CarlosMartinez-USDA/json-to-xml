...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="map/array[@key = 'pub_authors']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:choose>
            <xsl:when test="map/array[@key = 'pub_authors'] &lt; 1 ">
                <name type="corporate">
                    <namePart>
                        <xsl:value-of select="f:abbrvToName(map/string[@key = 'primary_station'])"/>
                    </namePart>
                </name>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="author-name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:for-each select="map[position()]">
            ...
            <!--affiliaton-->
            <xsl:choose>
                <!--tests for the presence of a station_id and unit_id-->
                <xsl:when test="(./string[@key = 'station_id']!= ' ') and (./string[@key = 'unit_id']!= ' ')">
                   
                    <xsl:analyze-string regex="^(\w\.\w\.\s\w+\s.*,)(.*?)(,)(.*)$" select="./string[@key = 'station_id']">
