...
-->
    ...
    <xd:doc>
        <xd:desc>
            <xd:p><xd:b>For local testing in Oxygen</xd:b>Comment this template or remove it
                entirely before putting into prodcution</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="data">
                ...
                <affiliation>
                    ...
                    <xsl:choose>
                        <xsl:when test="number(./string[@key = 'unit_id']) > 0">
                            <xsl:value-of select="local:unitNumToName(./string[@key = 'unit_id'],./string[@key = 'station_id'])"/>
                            <xsl:text>, </xsl:text>
                        </xsl:when>
