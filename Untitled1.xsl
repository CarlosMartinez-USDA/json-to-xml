...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="map/string[@key = ('pub_type_desc', 'pub_publication', 'citation')]" xpath-default-namespace="http://www.w3.org/2005/xpath-functions" mode="title">
        ...
        <xsl:variable name="publication" select="tokenize($typeOfTitle,',')[position()]"/>
        -*/--9++320
        <xsl:for-each-group select="$publication" group-by="$pub_series">
          ...
          <xsl:for-each select="current-group()">
            ...
            <xsl:when test="contains(map/string[@key='pub_publication'], document('./xml/usfs_treesearch.xml')/usfs:research/usfs:treeSeries/usfs:seriesPub/usfs:treePub">     
            <titleInfo>
