...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <!--output-->
    <xsl:output method="json" indent="yes" encoding="UTF-8" name="json"/>
    <xsl:output method="xml" indent="yes" encoding="UTF-8" name="mods"/><!--saxon:next-in-chain="fix_characters.xsl"/>-->
    ...
    <xsl:template match="data">
        <data>
            <xsl:result-document method="xml" omit-xml-declaration="yes" indent="yes" format="json"
                href="{$working_dir}/archive-files/A-{$original_filename}_{position()}.json">
                <xsl:value-of select="."/>
            </xsl:result-document>
        </data>
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            format="mods" href="{$working_dir}/mods-files/N-{$original_filename}_{position()}.xml">
            <mods version="3.7">
            ...
            <xsl:when test="count(map/string[@key = 'name']) = 0">
                ...
                <name type="corporate">
                    <namePart>
                        ...
                        <xsl:value-of
                            select="usfs:acronymToName(./array[@key = 'primary_station'])"/>
                        <xsl:value-of
    ...
    <xsl:template name="name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:if test="./string[@key = 'station_id'] != ''">
            <affiliation>
                ...
                <xsl:text>, </xsl:text>
                <xsl:choose>
                    <xsl:when test="./string[@key = 'unit_id'] castable as xs:double">
    ...
    <xsl:template name="dateIssued" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:choose>
            <xsl:when test="./string[@key = 'modified_on']">
                <xsl:apply-templates select="./string[@key = 'modified_on']"/>
            </xsl:when>
            <xsl:when test="map/string[@key = 'created_on']">
                <xsl:apply-templates select="./string[@key = 'created_on']"/>
            </xsl:when>
            <xsl:otherwise>
                <originInfo>
